...
//        return User.builder().username("admin").password(passwordEncoder.encode("{noop}admin")).authorities("admin").build();
        return User.builder().username("admin").password("{noop}admin").authorities("admin","auth:user:query").build();
    }
